...
* Editor
  1. We use an Ace Editor port for React [React-Ace](https://github.com/securingsincity/react-ace). All information about Editor settings is stored under `frontend/src/Editor.js` 
  2. The editor settings are passed down as props to the react-ace component and offer various configuration options check [this page](https://securingsincity.github.io/react-ace/) to experiment with different parameters.
  For eg: To change the theme import it from react-ace library
  `import "ace-builds/src-noconflict/theme-github";` and change the theme prop inside Editor.js
* Navbar : Navbar is configured under `frontend/src/Navbar.js`. It uses the default bootstrap navbar component.
* Tutorial:
  1. The Content for tutorial is stored inside the tutorial.json file, which is generated by the backend from tutorial.yml file.
  2. A tutorial consists of 3 sections- a title, content inside it and the code for the respective exercise. To add a new exercise append the three parameters inside the file at `src/backend/tutorial.yml`. For eg:
  ```
  ...
  Note: Please make sure you configure the code properly with proper string formatting, this can be done by entering the code inside the editor first and copying the outputed "code" parameter inside the POST request. This also ensures your code works properly. 
  3. The tutorial is handled by the TutorialCard component insidde `src/TutorialCard.js` with respective props for Heading and Title. The change for code happens automatically by the `tutFunc` function inside App.js bounded by the switching buttons. 
* InputBox - The InputBox is a separate component which has props passed down from App.js to effectively manage state for the input
* App.js : It houses all of our states and major functionalities(API Calls, Library Selection)
  1. States:
    ...
    * Exercise - Current exercise the user is on
    There are also states which hold information on status for various toggleable elements on the site
  ...
  2. Buttons for the tutorial are managed by `goRight` and `goLeft` functions, they iterate on the tutorial.json using `exercise` state.
  3. API:
    * POST Request to the backend server  is done via `handleClick` function. The request contains 3 parameters code, the input for it and libraries selected by the user. The response from this request is stored in the output stae.
  4. Custom Styling from buttons is stored under the style tag inside run-button class.
  5. The Output Section is inside the terminal class. All elements in this section are made via Bootstrap Cards. Tutorial Card Component is also inside the terminal class. 
  6. The Modal Pop up for library selection is also a part of this section. It holds the switches for libraries toggle.
  7. To Add a New library in the frontend -
  * Add a new `<Form.Check>` component with appropriate props and create a new state for the library.  For eg:
  ...
  <Form.Check
      ...
      checked={packageOn} />
  ```
  * Create a new onLibSelect function as follows:
  ```
  const onLibSelect = () => {
  ...
  The Styling for the app is stored under `frontend/src/App.css`. All styles are applied from a single CSS file.

### Backend
We use Flask to handle our backend operations, Docker-py is used to communicate with the docker engine on the server. The Dockerfile for the sandbox environment is stored under `backend/Docker`, it also contains the fpm.toml file for pre-building libraries on the container. 
Functions:
* edit_file(code, input, libs)
  1. It's responsible for editing the files with user code and user input.
  2. Editing fpm.toml file with library path is also done by this function. When adding a new library update this function with the library and it's path on the container. For eg:
  ```
  if "library-name" in libs:
      fpm["dependencies"] = {'library-name' : {'path' : "libraries/library-path"}}
  ```
* copy_to(src,dst,container) - It sends the file to our sandbox for execution.
* execute_code_in_container() - This function provides the commands for execution on the container. It builds the file and returns the output in form of a Tuple, containing exit code and output.
(exit code, output)
The output object is also a tuple of two values
(stdout,stderr)
For more info on docker commands read the [docker-py Documentation](https://docker-py.readthedocs.io/en/stable/index.html)

* API Endpoints 
  * '/run' : This is where all the request handling is processed. Data is received from the frontend and functions are performed on the data. 

  It returns a 202 status for Successful Execution, with the output of executed code.

### Docker
* The Dockerfile is located under `backend/Docker` directory, it contains all the information regarding the sandbox environment. It handles everything installing OS, required dependencies, fetching FPM.
* The fpm.toml file under this directory is used to pre build the libraries for faster execution in the container on server. Update this to get your libraries pre-built. 
* The main.f90 file is to initialize the pre-building of libraries via fpm. 
...
![image](https://user-images.githubusercontent.com/64147815/189549057-f0209128-86f7-4530-a3bf-ecdcf0832f1f.png)
